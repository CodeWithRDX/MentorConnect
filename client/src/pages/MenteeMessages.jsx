...
const MenteeMessages = () => {
  ...
  // Load contact list (mentors this mentee has chatted with)
  useEffect(() => {
  ...
  // Load conversation when active mentor changes
  useEffect(() => {
  ...
  const sendMessage = async () => {
    if (!activeMentorId) {
      toast('No mentor selected', 'error');
      return;
    }
    if (!message.trim()) return;
    setSending(true);
    try {
      const res = await api.post('/messages', { to: activeMentorId, body: message.trim() });
                ...
                <div className="h-64 border border-neutral-200 dark:border-neutral-700 rounded-lg p-3 bg-white dark:bg-neutral-900 overflow-y-auto text-sm space-y-2">
                  {loadingMessages ? (
                    ...
                    <div className="flex items-center justify-center h-full text-neutral-500 text-xs">
                      Choose a mentor from the list to view your messages.
                    </div>
                  ) : messages.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-neutral-500">
